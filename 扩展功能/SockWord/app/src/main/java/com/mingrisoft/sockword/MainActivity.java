...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, SynthesizerListener, RadioGroup.OnCheckedChangeListener {
    ...
    float x2 = 0;
    float y2 = 0;
    private int [] images; //用于加载图片
    private LinearLayout layoutBg;  //显示背景图片
    ...
    protected void onCreate(Bundle savedInstanceState) {
         ...
         * 随机获取背景图片并显示
         * */
        TypedArray ar =getResources().obtainTypedArray(R.array.imageArray); //获取图片数组
        layoutBg = (LinearLayout) findViewById(R.id.layout_add_bg);
        int len = ar.length();  //获取数组的长度
        images = new int[len];   //初始化加载图片的数组
        //for循环把图片读取出来
        for (int i = 0; i < len; i++){
            images[i] = ar.getResourceId(i, 0);
        }
        Random rand = new Random();          //初始化随机数
        int pos = rand.nextInt(images.length);   //随机获取出小于数组长度的数
        layoutBg.setBackgroundResource(images[pos]);   //把图片显示出来
